...
<PackageManagerTabs command="install with-react-native-oss-notice" />
...
<PackageManagerTabs command="react-native with-oss-notice" />
...
:::warning
`with-react-native-oss-notice` only detects licenses for dependencies listed in the app's `package.json`.   It won't include dependencies used only in monorepo packages unless they are also in `package.json`. To include more licenses, add those dependencies to the app's `package.json`.  
:::